...
function init()
{
  ...
  //Getting the landing page URL for the "Custom" option.
  geturl(item.location, function(locationres){
	if((!(locationres==null))&&(!(locationres==undefined)))
	{	Cu.reportError("locationres: " +  locationres);
		locationres = locationres.replace(/^[\w\-]+:\/+(?:www\.)?/, ""); //getting the domain name of the landing page
		Cu.reportError("locationres: " +  locationres);
		let suffix = "^$image,domain=~"+ locationres; //Adding filter options to the filter
		Cu.reportError("URL: " + locationres);
		E("customPattern").value = locationres + suffix;		
	}else 	
	{	Cu.reportError("location: " + item.location);
		E("customPattern").value = item.location;
	}
	});
