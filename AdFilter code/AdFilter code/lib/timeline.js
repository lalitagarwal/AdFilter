...
let TimeLine = exports.TimeLine = {
  ...
  log: function(/**String*/ message, /**Boolean*/ _forceDisplay)
  {
	...
	Cu.reportError("[" + now + "] ABP timeline: " + message + padding.join("") + "\t (" + diff + ")\n");
  },
  ...
  leave: function(message, asyncAction)
  {
    ...
    if (nestingCounter <= 0)
    {
      if (firstTimeStamp !== null)
		...
		Cu.reportError("ABP timeline: Total time elapsed: " + Math.round(Date.now() - firstTimeStamp) + "\n");
	  firstTimeStamp = null;
      lastTimeStamp = null;
    }
    ...
    if (typeof asyncAction != "undefined")
    {
      if (asyncAction in asyncActions)
		...
		Cu.reportError("ABP timeline: Warning: Async action " + asyncAction + " already executing\n");
  ...
  asyncStart: function(/**String*/ asyncAction)
  {
    if (asyncAction in asyncActions)
    {
      ...
      if ("currentStart" in action)
		...
		Cu.reportError("ABP timeline: Warning: Processing reentered for async action " + asyncAction + "\n");
      action.currentStart = Date.now();
    }
    else
      dump("ABP timeline: Warning: Async action " + asyncAction + " is unknown\n");
	  Cu.reportError("ABP timeline: Warning: Async action " + asyncAction + " is unknown\n");
  },
  ...
  asyncEnd: function(/**String*/ asyncAction)
  {
    if (asyncAction in asyncActions)
    {
      ...
      if ("currentStart" in action)
      {
		...
		Cu.reportError("ABP timeline: Warning: Processing not entered for async action " + asyncAction + "\n");
    }
    else
      dump("ABP timeline: Warning: Async action " + asyncAction + " is unknown\n");
	  Cu.reportError("ABP timeline: Warning: Async action " + asyncAction + " is unknown\n");
  ...
  asyncDone: function(/**String*/ asyncAction)
  {
    if (asyncAction in asyncActions)
    {
      ...
      if ("currentStart" in action)
		...
		Cu.reportError("ABP timeline: Warning: Still processing for async action " + asyncAction + "\n");
	  ...
	  Cu.reportError("[" + now + "] ABP timeline: " + message + padding.join("") + "\t (" + action.total + "/" + diff + ")\n");
    }
    else
      dump("ABP timeline: Warning: Async action " + asyncAction + " is unknown\n");
	  Cu.reportError("ABP timeline: Warning: Async action " + asyncAction + " is unknown\n");
