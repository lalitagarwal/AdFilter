/*
 * This file is part of the AdFilter Plus,
 * Copyright (C) 2006-2012 Eyeo GmbH
 *
 * AdFilter Plus is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 3 as
 * published by the Free Software Foundation.
 *
 * AdFilter Plus is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with AdFilter Plus.  If not, see <http://www.gnu.org/licenses/>.
 */
...
let Policy = exports.Policy =
{
  ...
  processNode: function(wnd, node, contentType, location, collapse)
  {
	...
	if (!match && Prefs.enabled)
    {
	  //If it is an image, get the href attribute of the parent
	  if(node.nodeName=="IMG"){
		locationText=node.parentNode.getAttribute('href');
	 }
	 //Getting the adurl string from the locationText
	 if(!locationText==null)
		var adurl = (/adurl=(.*?)$/).exec(locationText)[1];
	 if((!(adurl==null))&&(!(locationText==null)))
		match = defaultMatcher.matchesAny(adurl, Policy.typeDescr[contentType] || "", docDomain, thirdParty);
	 else
		match=null;
	  if (match instanceof BlockingFilter && node.ownerDocument && !(contentType in Policy.nonVisual))
      {
		Cu.reportError("Match is " + match + " Locationtext: " +  locationText);		
		let prefCollapse = (match.collapse != null ? match.collapse : !Prefs.fastcollapse);
        //If it is an iframe or embed tag, allow it to load
		if((node.nodeName=="IFRAME")||(node.nodeName=="EMBED")){
			match=null;
		}
		else if (collapse || prefCollapse)
          schedulePostProcess(node);
      }
	  //If match on the src url doesnt occur, get the landing page url and check for a match on that instead
	  else{
		//Only If it is not a script tag (.js files), get the landing page URL
		if(!(node.nodeName=="SCRIPT")){
			//calling function to get the landing page url
			geturl(locationText, function(locationres){
			
			if((!(locationres==null))&&(!(locationres==undefined))){
				match = defaultMatcher.matchesAny(locationres, Policy.typeDescr[contentType] || "", docDomain, thirdParty);
				if (match instanceof BlockingFilter && node.ownerDocument && !(contentType in Policy.nonVisual))
				{
					Cu.reportError("Match is " + match + " Locationtext: " +  locationText);		
					let prefCollapse = (match.collapse != null ? match.collapse : !Prefs.fastcollapse);
		 ...
		 //If it is a script tag, allow it to load
		if((node.nodeName=="SCRIPT")){
			match=null;   
		}
		//If it is an embed tag, allow it to load
	...
	//return false for blocking, true otherwise
...
//Returns the landing page url from a Source URL by sending HTTP requests
function geturl(url, callback)
{
    ...
    try
    {
      //initialising the xmlhttprequest object
	  request = Cc["@mozilla.org/xmlextras/xmlhttprequest;1"].createInstance(Ci.nsIXMLHttpRequest);
      request.mozBackgroundRequest = true; 
	  request.open("GET", url, true); //true for asynchronous requests
	  request.setRequestHeader("Access-Control-Expose-Headers", "Location"); //[optional]requesting to include Location field in the response header
	  request.setRequestHeader("Access-Control-Allow-Origin", "*"); //[optional]allowing cross domain requests to be sent
    ...
    catch (e)
    {
	  ...
	  // Override redirect limit from preferences, user might have set it to 1. Set it to 0, this will allow redirects to occur.
	...
	//Called on receiving the response of the request
	request.onreadystatechange = function() {
		//Getting the Landing page URL from the Location field in the response header.
		locationres= request.getResponseHeader("Location");
		callback(locationres);
	}
}
...
{
  classDescription: "AdFilter Plus content policy",
  ...
  init: function()
  {
    ...
    onShutdown.add(function()
    {
	  ...
	  //Services.obs.removeObserver(this, "document-element-inserted", true);
